  ...
  <div class="column-wrap">
    ...
    >
      <el-form ref="ruleFormRef" :model="form" :rules="rules" :label-position="labelPosition" label-width="120">
        <el-form-item label="书签栏名称" prop="colName">
...
const rules = reactive<FormRules>({
  colName: [{ required: true, message: '请输入书签栏名称', trigger: 'blur' }]
});
...
const edit = (row: any) => {
  form.id = row.id;
  form.colName = row.colName;
  dialogTitle.value = '编辑书签栏';
  dialogVisible.value = true;
};

const del = async (id: number) => {
  let result = await midApi.delCol(id);
  if (result) {
    ElMessage({
      message: '删除成功',
      type: 'success',
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      if (dialogTitle.value === '新增专栏') {
        const data = await midApi.addCol({ ...form });
        if (data) {
          ElMessage({
            message: '新增成功',
            type: 'success',
        ...
        const data = await midApi.updateCol({ ...form });
        if (data) {
          ElMessage({
            message: '更新成功',
            type: 'success',
